...
var renderTemplate = function (blob, argsObject) {
  ...
  <html id="test">
    <head>
      <base target="_top">
      <meta charset="UTF-8">
      <meta name="renderTemplate" content="Budget Render Template">
...
var appList = function (e) {
  ...
  <html id="appList">
    <head>
      <base target="_top">
      <meta charset="utf-8">
      <meta name="appList" content="Budget Function List">
      <meta name=viewport content="width=device-width, initial-scale=1">
      <link href="https://fonts.googleapis.com/css?family=Acme" rel="stylesheet">
    </head>
    <body>
    <? var appList = App.appSort(9) ?>
    <? var dropList = appList.map((item) => {
        return "<option>" + item + "</option>"

    }).join("") ?>
    <? var result = JSON.stringify(dropList) ?>
    <? var appUrl = App.getUrl(ScriptApp) + "?func=" ?>
    <nav>
      <div class="row">
              ...
              <div class="black" id="seoData">
                <div class="row">
                  <div class="col s10 l10 m10 card-panel push-s1 push-l1 push-m1">
                    <div class="container row valign-wrapper"><?!= App.rule() ?></div>
